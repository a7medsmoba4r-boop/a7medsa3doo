...
function updateDynamicHeight(){
  const inProgressItems = inprogressTasks.children.length;
  const completedItems = completedTasks.children.length;
  const baseHeight = 50;
  const perItem = 40;
  inprogressTasks.style.height = `${Math.min(baseHeight + perItem * inProgressItems, MAX_HEIGHT)}px`;
  completedTasks.style.height = `${Math.min(baseHeight + perItem * completedItems, MAX_HEIGHT)}px`;
}
...
function handleTaskClick(e){
  ...
  if(e.target.classList.contains("delete-btn")){ li.remove(); updateCounts(); }
}
...
completedTasks.addEventListener("click",(e)=>{
  ...
  if(e.target.classList.contains("delete-btn")){
    ...
    inprogressTasks.appendChild(liInProgress);
    updateCounts();
  }else if(e.target.classList.contains("done-btn")){ li.remove(); updateCounts(); }
});
